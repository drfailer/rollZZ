...
void PeerManager::readBroadcastDatagram()
{
    while (broadcastSocket.hasPendingDatagrams()) {
        ...
        QHostAddress ip4Address(senderIp.toIPv4Address(&conversionOK));
        QString ip4String = "";
        if (conversionOK)
            ip4String = ip4Address.toString();

        //std::cout << localhost.toString().toStdString() << " and " << ip4String.toStdString() << std::endl;
        //std::cout << senderServerPort << " and " << serverPort << std::endl;
